...
function Dialog(position, ground, squish, rotate) {
    ...
    if (squish) {
        this._fall = new Gravitas.Fall(ground, 5000, 367, 10);
    } else if (rotate) {
        this._fall = new Gravitas.Fall(ground, 686, 404, 37);
    } else {
        this._fall = new Gravitas.Fall(ground, 5000, 180, 20);
    }
    ...
    this._animation = Gravitas.createAnimation(this._fall, this._update.bind(this));
}
...
function FallingDialogsDemo(element, squishy, rotate) {
    ...
    controls.addText(squishy ? 'Squishy Falling Dialog' : (rotate ? 'Rotating Falling Dialog' : 'Falling Dialog'));
